  ...
  public struct Transition: Hashable {
    ...
    public func isFireable(from marking: [Place: Int]) -> Bool {
      for place in preconditions{
        switch preconditions[place.key]! { // pour chaque préconditions
          case .regular(let value):
            if marking[place.key]! < value { // vérifie si la transition est tirable pour la marquage donné
              return false
            }
          case .inhibitor:
            if marking[place.key] != 0 { // vérifie si la transition est tirable pour la marquage donné
                return false
            }
          }
      }
    ...
    public func fire(from marking: [Place: Int]) -> [Place: Int]? {
      if isFireable(from: marking){
        var newMarking = marking // ancien marking dans new marking

        //precondition
        for place in preconditions {
          switch preconditions[place.key]! { // pour chaque précondition
            case .regular(let value):
              newMarking[place.key]! -= value // soustrait la valeur à la place
              break
            case .inhibitor:
              break
            }
      ...
      for place in postconditions{
        switch postconditions[place.key]! { // pour chaque postcondition
          case .regular(let value):
            newMarking[place.key]! += value // ajoute la valeur à la place
            break
          case .inhibitor:
            break
        }
