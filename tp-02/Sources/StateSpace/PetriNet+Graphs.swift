extension PetriNet {
    ...
    -> CoverabilityNode<Place>?
  {
    ...
    if let successor = created.first(where : { other in other.marking == nextmarking }) {
      node.successors[transition] = successor
    } else if (predecessors.contains(where: { other in other.marking < nextmarking }) || node.marking < nextmarking){
      var pred = predecessors.first(where: { other in other.marking < nextmarking })
      if(pred == nil){
        pred = node
      }

      for place in Place.allCases{
          if((pred!).marking[place] < nextmarking[place]){    
