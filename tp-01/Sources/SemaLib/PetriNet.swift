...
public struct PetriNet {
  ...
  public func isFireable(_ transition: Transition, from marking: Marking) -> Bool {
    ...
    // La transition est tirable pour toute place de l'ensemble des places tel que le
  ...
  public func fire(_ transition: Transition, from marking: @escaping Marking) -> Marking? {
    ...
    // Si la transition est tirable depuis le marquage dans lequel on se trouve
    if(self.isFireable(transition, from: marking)) {
      return {
        // On produit un nouveau marquage (place) donné par marquage initial - entrée(p,t) + sortie(p,t)
        (place) -> Nat in
